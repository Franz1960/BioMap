...
namespace BioMap.ImageProc
{
  ...
  internal class PatternMatchingProcessor<TPixelBg, TPixelFg> : ImageProcessor<TPixelBg>
      where TPixelBg : unmanaged, IPixel<TPixelBg>
      where TPixelFg : unmanaged, IPixel<TPixelFg>
  {
    /// <summary>
    /// Initializes a new instance of the <see cref="PatternMatchingProcessor{TPixelBg, TPixelFg}"/> class.
    /// </summary>
    ...
    public PatternMatchingProcessor(
      ...
      this.Blender = PatternMatchingPixelBlender<TPixelBg>.Instance;
    ...
    /// <summary>
    /// A <see langword="struct"/> implementing the draw logic for <see cref="PatternMatchingProcessor{TPixelBg,TPixelFg}"/>.
    /// </summary>
  ...
  /// <summary>
  /// A pixel blender that implements pattern matching.
  /// </summary>
  public class PatternMatchingPixelBlender<TPixel> : PixelBlender<TPixel> where TPixel : unmanaged, IPixel<TPixel>
  {
    /// <summary>
    /// Gets the static instance of this blender.
    /// </summary>
    public static PatternMatchingPixelBlender<TPixel> Instance { get; } = new PatternMatchingPixelBlender<TPixel>();
    ...
    protected override void BlendFunction(Span<Vector4> destination, ReadOnlySpan<Vector4> background, ReadOnlySpan<Vector4> source, float amount) {
      for (int i = 0; i < destination.Length; i++) {
        destination[i] = ((source[i] - background[i]).Length() < 0.5f) ? new Vector4(0f, 0f, 0f, 1f) : new Vector4(1f, 0.4f, 0.4f, 1f);
      }
    }
