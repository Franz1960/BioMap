...
namespace FSofTUtils.Geometry.PolylineSimplification
{
  ...
  public class PointList
  {
    ...
    public void ReumannWitkam(double dWidth) {
      int iStart = 0;
      int iNext = 1;
      while (pt[iStart].Equals(pt[iNext]) && iNext < Length)
        iNext++;      // ungleichen Punkt suchen
      int iTest = iNext + 1;
      dWidth *= dWidth;
      ...
      while (iTest < Length) {
        ...
        if (bPointIsValid) {      // p2 bleibt erhalten
          ...
          iStart = iTest;
          iNext = iStart + 1;
          while (iNext < Length && pt[iStart].Equals(pt[iNext]))
            iNext++;      // ungleichen Punkt suchen
          iTest = iNext + 1;
          while (iTest < Length && pt[iNext].Equals(pt[iTest]))
            iTest++;       // ungleichen Punkt suchen
        } else {
