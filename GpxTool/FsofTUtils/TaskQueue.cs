...
namespace FSofTUtils
{
  ...
  /// </para>
  /// <para>
  /// Der Taskworker kann bei Bedarf CancellationtokenSource.Token.ThrowIfCancellationRequested(); ausführen. Sollt ein anderer Task bereits eine Cancel-Anforderung gesetzt haben, 
  /// wird damit im akt. Task eine Exception ausgelöst.
  /// </para>
  /// <para>
  /// Falls in einem Task eine Exception (mit einem Infotext) ausgelöst wurde, wird <see cref="ExeptionMessage"/> mit diesem Text gesetzt. Nach <see cref="Wait4EmptyQueue"/>()
  /// sollte der Wert abgefragt werden!
  /// </para>
  /// <para>
  ...
  public class TaskQueue : IDisposable
  {
    ...
    readonly object exeptionlocker = new object();
    ...
    void SetExeptionMessage(string msg) {
     ...
     * 
     * Der Taskworker kann bei Bedarf CancellationtokenSource.Token.ThrowIfCancellationRequested(); ausführen!
     */
    ...
    public void StartTask<TPROGRESS, TEND>(Func<CancellationTokenSource, IProgress<TPROGRESS>, TEND> taskworker,
      ...
      Task t = Task.Run(() => {
        try {
          ...
          TEND result = taskworker(CancellationtokenSource, taskprogress);
          taskend?.Invoke(result);   // über das Ende und das Ergebnis informieren
        } catch (Exception ex) {
          SetExeptionMessage(ex.Message);
        }
        Taskpost();
      });
    }
    ...
    public void StartTask<TDAT1, TPROGRESS, TEND>(TDAT1 taskdata1,
                                                  Func<TDAT1, CancellationTokenSource, IProgress<TPROGRESS>, TEND> taskworker,
                                                  IProgress<TPROGRESS> taskprogress = null,
                                                  Action<TEND> taskend = null) {
      Taskpre();
      Task t = Task.Run(() => {
        try {
          CancellationtokenSource.Token.ThrowIfCancellationRequested();
          TEND result = taskworker(taskdata1, CancellationtokenSource, taskprogress);
          taskend?.Invoke(result);   // über das Ende und das Ergebnis informieren
        } catch (Exception ex) {
          SetExeptionMessage(ex.Message);
        }
    ...
    public void StartTask<TDAT1, TDAT2, TPROGRESS, TEND>(TDAT1 taskdata1,
                                                         TDAT2 taskdata2,
                                                         Func<TDAT1, TDAT2, CancellationTokenSource, IProgress<TPROGRESS>, TEND> taskworker,
                                                         IProgress<TPROGRESS> taskprogress = null,
                                                         Action<TEND> taskend = null) {
      Taskpre();
      Task t = Task.Run(() => {
        try {
          CancellationtokenSource.Token.ThrowIfCancellationRequested();
          TEND result = taskworker(taskdata1, taskdata2, CancellationtokenSource, taskprogress);
          taskend?.Invoke(result);   // über das Ende und das Ergebnis informieren
        } catch (Exception ex) {
          SetExeptionMessage(ex.Message);
        }
    ...
    public void StartTask<TDAT1, TDAT2, TDAT3, TPROGRESS, TEND>(TDAT1 taskdata1,
                                                                TDAT2 taskdata2,
                                                                TDAT3 taskdata3,
                                                                Func<TDAT1, TDAT2, TDAT3, CancellationTokenSource, IProgress<TPROGRESS>, TEND> taskworker,
                                                                IProgress<TPROGRESS> taskprogress = null,
                                                                Action<TEND> taskend = null) {
      Taskpre();
      Task t = Task.Run(() => {
        try {
          CancellationtokenSource.Token.ThrowIfCancellationRequested();
          TEND result = taskworker(taskdata1, taskdata2, taskdata3, CancellationtokenSource, taskprogress);
          taskend?.Invoke(result);   // über das Ende und das Ergebnis informieren
        } catch (Exception ex) {
          SetExeptionMessage(ex.Message);
        }
    ...
    public void StartTask<TDAT1, TDAT2, TDAT3, TDAT4, TPROGRESS, TEND>(TDAT1 taskdata1,
                                                                       ...
                                                                       TDAT3 taskdata3,
                                                                       TDAT4 taskdata4,
                                                                       Func<TDAT1, TDAT2, TDAT3, TDAT4, CancellationTokenSource, IProgress<TPROGRESS>, TEND> taskworker,
                                                                       IProgress<TPROGRESS> taskprogress = null,
                                                                       Action<TEND> taskend = null) {
      Taskpre();
      Task t = Task.Run(() => {
        try {
          CancellationtokenSource.Token.ThrowIfCancellationRequested();
          TEND result = taskworker(taskdata1, taskdata2, taskdata3, taskdata4, CancellationtokenSource, taskprogress);
          taskend?.Invoke(result);   // über das Ende und das Ergebnis informieren
        } catch (Exception ex) {
          SetExeptionMessage(ex.Message);
        }
    ...
    /// <para>Danach ist der Test von <see cref="ExeptionMessage"/> sinnvoll!</para>
    /// </summary>
    /// <returns></returns>
